package com.guzey.intellistart.interviewplanning.model.booking.validation;

import com.guzey.intellistart.interviewplanning.exceptions.BookingException;
import com.guzey.intellistart.interviewplanning.exceptions.BookingLimitException;
import com.guzey.intellistart.interviewplanning.exceptions.SlotException;
import com.guzey.intellistart.interviewplanning.exceptions.UserException;
import com.guzey.intellistart.interviewplanning.model.bookinglimit.BookingLimitService;
import com.guzey.intellistart.interviewplanning.model.candidateslot.CandidateSlot;
import com.guzey.intellistart.interviewplanning.model.interviewerslot.InterviewerSlot;
import com.guzey.intellistart.interviewplanning.model.interviewerslot.InterviewerSlotService;
import com.guzey.intellistart.interviewplanning.model.booking.Booking;
import com.guzey.intellistart.interviewplanning.model.period.Period;
import com.guzey.intellistart.interviewplanning.model.period.PeriodService;
import com.guzey.intellistart.interviewplanning.model.period.services.TimeService;
...
public class BookingValidator {
  ...
  public void validateUpdating(Booking updatingBooking, Booking newDataBooking)
    ...
    if (periodDuration != BOOKING_PERIOD_DURATION_MINUTES) {
      throw new SlotException(SlotException.SlotExceptionProfile.INVALID_BOUNDARIES);
    }

    if (newDataBooking.getSubject().length() > SUBJECT_MAX_SIZE) {
      throw new BookingException(BookingException.BookingExceptionProfile.INVALID_SUBJECT);
    }
    if (newDataBooking.getDescription().length() > DESCRIPTION_MAX_SIZE) {
      throw new BookingException(BookingException.BookingExceptionProfile.INVALID_DESCRIPTION);
    }
    ...
    if (!interviewerSlotDate.equals(newCandidateSlot.getDate())) {
      throw new BookingException(BookingException.BookingExceptionProfile.SLOTS_NOT_INTERSECTING);
    }
